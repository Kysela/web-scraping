...
usage() {
	echo -e "\nThis script can be used to get latest sourceforge" 
	echo -e "\nproject updates and send them formatted to Telegram channels or groups" 
	...
	echo -e "-p | --project <project name> - Name of the sourceforge project"
...
if [ -z "$PROJECT" ]; then
	echo "Please set sourceforge project name to search for!"
	exit 1
fi
...
if [ -z "$CONFIG" ]; then
    if [ "$DEVICE" ]; then
	CONFIG="${PROJECT}_${DEVICE}_${CHAT_ID}.config";
	else
	CONFIG="${PROJECT}_${CHAT_ID}.config";
	fi
fi

CHANGELOG_TABLE=(
"dirtyunicorns" "mido" "https://raw.githubusercontent.com/dracarys18/DU_OTA/master/changelog./changelog.txt"
"syberiaos" "mido" "https://raw.githubusercontent.com/syberia-project/official_devices/master/a-only/mido.changelog"
"syberiaos" "whyred" "https://raw.githubusercontent.com/syberia-project/official_devices/master/a-only/whyred.changelog"
"pixys-os" "mido" "https://raw.githubusercontent.com/PixysOS-Devices/changelogs/master/mido.txt"
"pixys-os" "whyred" "https://raw.githubusercontent.com/PixysOS-Devices/changelogs/master/whyred.txt"
"aosipderp-fest4whyred" "whyred" "https://gitlab.com/isyedsajid/derpfest4whyred/blob/master/Changelogs.txt"
"illusionproject" "mido" "https://raw.githubusercontent.com/AOSiP-Devices/Updater-Stuff/master/mido/changelog"
"illusionproject" "whyred" "https://github.com/AOSiP-Devices/Updater-Stuff/blob/master/whyred/changelog"
)
...
local_current_array=()
local position=0;
for i in "${CHANGELOG_TABLE[@]}"; do
if (( position == 2 )); then
local_current_array+=("$i")
if [ "$PROJECT" = "${local_current_array[0]}" ] && [ "$DEVICE" = "${local_current_array[1]}" ]; then
echo "${local_current_array[2]}"
return 0;
fi
position=0;
local_current_array=()
...
echo "*We will try to add it soon*"
return 1;
}
...
if ! website_exists "https://sourceforge.net/projects/${PROJECT}"; then
if ! website_exists https://sourceforge.net; then
echo "Unable to locate sourceforge, may be it is down?";
exit 1;
fi
echo "Unable to locate specified project, are you sure that $PROJECT exists?";
exit 1;
fi
...
local bytes_size="`curl -k -L -I -s "$1" | grep -i Content-Length | awk '{print $2}'`";
echo "`./human_size "$bytes_size"`";
}

get_sourceforge_info() {
local sourceforge_link="$1";
case $sourceforge_link in
*"/download") sourceforge_link="${sourceforge_link%/*}"; ;;
esac
local sourceforge_link_basename="${sourceforge_link##*/}";
local sourceforge_dirname="{sourceforge_link%/*}"; 
case $sourceforge_dirname in
*"/files") ;;
*) sourceforge_link="${sourceforge_link%/*}"; ;;
esac
local found_filename=false;
local found_filesize=false
process_webpage "${sourceforge_link}" | while read -r line; do
if [ "$found_filesize" != "finished" ]; then
case $line in
*">${sourceforge_link_basename}<"*) found_filesize=true; ;;
esac
if $found_filesize; then
if [[ "$line" == *"\"files_size_h\""* ]]; then
local link=$(process_webpage "${1}" | parse_download_url | sed -e 's/\(?r=\).*\(ts=\)/\1\2/' -e 's/\&amp;/\&/g' -e 's/r=ts=/r=\&ts=/g')
local link_size="`get_download_link_size "$link"`";
if [ "${link_size}" ] && [ "$link_size" != "0B" ]; then
echo "<b>FileSize</b>: ${link_size}";
else
echo "<b>FileSize</b>: `echo "$line" | parse_double_argument_html_tag`";
fi
found_filesize="finished";
elif [[ "$line" == *"\"files_date_h\""* ]]; then
echo "<b>BuildDate</b>: `echo "$line" | parse_triple_argument_html_tag | sed 's/"//g'`";
fi
fi
fi
if [[ "$line" == "net.sf.files = {"* ]]; then
echo "$line" | sed -e $'s/},/\},\\n/g' -e 's/\\//g' -e 's/\", /\",\n/g' -e 's/, \"/,\n \"/g' -e 's/\": {\"/\": {\n\"/g' -e 's/net.sf.files = {//g' | while IFS= read -r json; do
parse_json_pure_bash "${json}"
if [ "${found_filename}" = "false" ] && [ "${JSON_KEY}" = "${sourceforge_link_basename}" ]; then
found_filename=true;
fi
if $found_filename; then
if [ "${JSON_KEY}" = "md5" ]; then
echo "<b>MD5</b>: <code>${JSON_VALUE}</code>";
elif [ "${JSON_KEY}" = "sha1" ]; then
echo "<b>SHA1</b>: <code>${JSON_VALUE}</code>"
break;
fi
fi
done
...
parse_sourceforge_project_files() {
if [ -z "$1" ]; then
return 1;
fi
local LINKS="`process_webpage https://sourceforge.net/projects/${1}/files/ | parse_html`";
for i in $LINKS; do
([ "$i" != "/projects/${1}/files/" ] && [ "$i" != "/projects/${1}/files/latest/download" ] && [[ "$i" == "/projects/${1}/files/"* ]] && [[ "$i" != *"/stats/timeline" ]]) || continue;
echo "https://sourceforge.net${i}";
done
}

parse_sourceforge_files_object() {
local LINKS="`process_webpage ${1} | parse_html`";
for i in $LINKS; do
([[ "$i" != *"/download" ]] || [[ "$i" == *"latest/download" ]]) && continue;
echo "${i}";
done
}
...
local zip_id="`get_zip_id "${2}/download"`";
if [ "${GENERATE_CONFIG}" = "new" ]; then
./database -f "$CONFIG" -s "${2}_${zip_id}";
return 0;
elif [ "${GENERATE_CONFIG}" = "true" ]; then
./database -f "${CONFIG}.tmp" -s "${2}_${zip_id}";
fi
if [ -f "$CONFIG" ]; then
if ./database -f "$CONFIG" -c "${2}_${zip_id}"; then
return 0;
fi
./database -f "$CONFIG" -s "${2}_${zip_id}";
fi
...
<b>Download</b>: <a href=\"${2}/download\">`download_link_basename $2`</a>"


if [ "$3" ]; then
local file_type="unknown";
case $3 in
*".sha" | ".sha2" | ".sha256") file_type="sha"; ;;
*".md5sum" | *".md5") file_type="md5"; ;;
*"-changelog.txt" | *"-Changelog.txt" | *"-CHANGELOG.txt" | *"CHANGELOG.TXT") file_type="changelog"; ;;
esac

if [ "${file_type}" = "sha" ]; then
message+="

<b>SHA</b>: <a href=\"${3}/download\">Click here</a>";
elif [ "${file_type}" = "md5" ]; then
message+="

<b>MD5</b>: <a href=\"${3}/download\">Click here</a>";
elif [ "${file_type}" = "changelog" ]; then
message+="

<b>Changelog</b>: <a href=\"${3}/download\">Click here</a>";
fi
fi
...
<b>Changelog</b>: <a href=\"${4}/download\">Click here</a>";
;;
esac
fi
...
if [ "${CHAT_ID}" = "aospamido" ]; then
message+="
<b>Changelog</b>
https://raw.githubusercontent.com/dracarys18/DU_OTA/pa-quartz/changelog./changelog.txt
";
elif [ "${CHAT_ID}" = "dirtyunicorns" ]; then
message+="
<b>Changelog</b>
https://raw.githubusercontent.com/dracarys18/DU_OTA/master/changelog./changelog.txt
";
fi
...
local sourceforge_info="`get_sourceforge_info "${2}/download"`";
if [ "$sourceforge_info" ]; then
message+="
$sourceforge_info";
else
return 0;
...
local_device_array=(
"RedmiNote4X" "@rn4downloads"
"RedmiNote4" "@rn4downloads"
"mido" "@rn4downloads"
"whyred" "@whyreddownloads"
"RedmiNote5" "@whyreddownloads"
"RedmiNote5Pro" "@whyreddownloads"
"HMNote5Global" "@whyreddownloads"
"HMNote5Pro" "@whyreddownloads"
"HMNote4X" "@rn4downloads"
"HMNote4XGlobal" "@rn4downloads"
)
...
local i
local position=false;
for i in "${local_device_array[@]}" ; do
  if ! $position; then
  key=$i;
  position=true;
  continue;
  ...
  if check_device_link "$2" "$key"; then
  if [ "${CHAT_ID}" != "${value}" ]; then
...
`get_channel_tags $value`";
send_message_to_tg "$message_copy" "$value";
fi
fi
done
...
get_sourceforge_activity_links() {
local found_activity=false
local latest_activity_id="";
process_webpage "$1" | while read -r activity_link; do
if ! $found_activity; then
[ "$activity_link" = "<div class=\"activity\">" ] && found_activity=true;
continue;
fi
if [[ "$activity_link" == "<li id="* ]]; then
latest_activity_id=${activity_link#*\"}
latest_activity_id=${latest_activity_id%%\"*}
elif [[ "$activity_link" == "<a href="* ]] && [[ "$activity_link" == *"/download\">"* ]]; then
echo "activity_id=${latest_activity_id}";
activity_link="`echo "$activity_link" | parse_html`"
echo "$activity_link";
fi
done
}
...
WEBSITE="https://sourceforge.net/p/${PROJECT}/activity";
activity_array=(`get_sourceforge_activity_links "$WEBSITE"`)
zip_array=()
md5_array=()
sha_array=()
changelog_array=()
for parsed_link in "${activity_array[@]}"; do
case $parsed_link in
"activity_id="*)
continue;
;;
...
[[ "$parsed_link" != *"/$DEVICE/"* ]] && [[ "$device_verification_basename" != "${DEVICE}_"* ]] && [[ "$device_verification_basename" != "${DEVICE}-"* ]] && [[ "$device_verification_basename" != *"_${DEVICE}_"* ]] && [[ "$device_verification_basename" != *"_${DEVICE}-"* ]] && [[ "$device_verification_basename" != *"-${DEVICE}_"* ]] && [[ "$device_verification_basename" != *"-${DEVICE}-"* ]] && [[ "$device_verification_basename" != *"-${DEVICE}."* ]] && [[ "$device_verification_basename" != *"_${DEVICE}."* ]] && continue;
fi
